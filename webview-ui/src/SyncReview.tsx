...
function defaultResolution(kind: EpicSyncStateView['kind'] | StorySyncStateView['kind']): Resolution {
...
const initial: SyncState = {
    plan: null, provider: null, epicResolutions: {}, storyResolutions: {},
...
export function SyncReview() {
    ...
    const handleApply = useCallback(() => {
        ...
        syncApi.postMessage({ type: 'apply', epicResolutions, storyResolutions });
    }, [state]);
        ...
        <div className="sync-root">
            <div className="sync-header">
                ...
                <div className="sync-subtitle">
                    {plan.epics.length + plan.stories.length} items checked
            ...
            {(localOnly.length > 0 || localOnlyStories.length > 0) && (
            ...
            {(remoteOnly.length > 0 || remoteOnlyStories.length > 0) && (
                <section className="sync-section">
            ...
            {/* ── Conflicts ──────────────────────────────────────────────── */}
            {(conflicts.length > 0 || conflictStories.length > 0) && (
                <section className="sync-section">
            ...
            {(plan.unpushedEpicIds.length > 0 || plan.unpushedStoryIds.length > 0) && (
                <section className="sync-section sync-section--muted">
                    <h3 className="sync-section-title">Not yet pushed (excluded from sync)</h3>
                    <ul className="sync-unpushed-list">
                        {[...plan.unpushedEpicIds, ...plan.unpushedStoryIds].map((id) => (
